...
class FormularAdminController extends AbstractController
{
    ...
    /**
     * @Route("/toogleStatus", name="toogle_status_turnier")
     * @param int $turnierId
     */
    public function deactivateTurnier(Request $request)
    {
        if ($request->isXmlHttpRequest()) {
            ...
            $turnier = $this->getDoctrine()->getRepository(TurnierForm::class)->find($request->request->get('id'));
            if ($turnier->isActive()) {
                $turnier->setIsActive(false);
            } else {
                $turnier->setIsActive(true);
            }
            $em = $this->getDoctrine()->getManager();
            $em->persist($turnier);
            $em->flush();
            return new JsonResponse('New Status is: ' . $turnier->isActive());
        }
